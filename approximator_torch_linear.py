    ...
    def train(self, env, total_episode):
        ...
        for episode in range(total_episode):
            ...
            # begin answer
                ...
                new_state, reward, flag, info = env.step(action)
                # flag = 0 reward = 1
                # flag = 1 reward = -1
                reward = - flag * 2 + 1
                loss = F.smooth_l1_loss(reward + self.gamma * self.model(torch.Tensor(new_state)).max(), self.model(torch.Tensor(state))[action])
