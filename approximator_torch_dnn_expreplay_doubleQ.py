...
class Approximator:
    ...
    def __init__(self,
                 ...
                 double_QLearning_exchange_cnt=10):
        ...
        # begin answer
        self.double_QLearning_exchange_cnt = double_QLearning_exchange_cnt
        self.optimzer = torch.optim.SGD(self.model.parameters(), lr=self.learning_rate)
        if double_QLearning:
            self.v_model = Model(self.state_size, self.action_size)
    ...
    def online_training(self, state, action, reward, new_state, done):
        ...
        if self.double_QLearning:
            loss = F.smooth_l1_loss(rewards + self.gamma * self.v_model(torch.Tensor(next_states)).max(axis=1).values, self.model(torch.Tensor(states)).gather(1, actions.reshape(-1,1)).flatten())
        else:
    ...
    def train(self, env, total_episode):
        ...
        for episode in range(total_episode):
            ...
            if self.double_QLearning and episode % self.double_QLearning_exchange_cnt == 0:
                self.v_model.load_state_dict(self.model.state_dict())
            # end answer
